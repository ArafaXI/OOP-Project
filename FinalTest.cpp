...
// To compile: FinalTest.cpp Menu.cpp PrintedItem.cpp Item.cpp Book.cpp
// Library.cpp Member.cpp Magazine.cpp Ebook.cpp DigitalItem.cpp -o sfml-menu
// -lsfml-graphics -lsfml-window -lsfml-system To run: /sfml-menu
    ...
    if (isSelected && event.type == sf::Event::TextEntered) {
      if (event.text.unicode == 8 && !inputString.empty()) {  // Backspace
        inputString.pop_back();
                 ...
                 event.text.unicode > 31) {  // Printable chars
...
class Button {
 public:
  Button(float x, float y, float width, float height,
         const std::string &labelText, sf::Font &font) {
...
int main() {
  ...
  sf::Vector2i windowPosition((desktop.width - 800) / 2,
  ...
  Textbox publicationDateBox(100, 400, 600, 40, font,
  ...
  Textbox magazinePageCountBox(100, 220, 600, 40, font,
                               "Enter Magazine Page Count");
  Textbox magazineBindingTypeBox(100, 280, 600, 40, font,
                                 "Enter Magazine Binding Type");
  Textbox magazinePublicationDateBox(100, 340, 600, 40, font,
                                     "Enter Magazine Publication Date");
  ...
  Textbox ebookPublicationDateBox(100, 400, 600, 40, font,
                                  "Enter Ebook Publication Date");
  ...
  // Create a button for "Back" in the display items section
  Button backDisplayItemsButton(100, 460, 200, 50, "Back", font);
  ...
  while (window.isOpen()) {
    ...
    while (window.pollEvent(event)) {
      ...
      if (inAddMemberMenu) {
        ...
        if (event.type == sf::Event::MouseButtonPressed) {
          if (addLibraryButton.isClicked(window)) {
            ...
            library.addItem(new Book(title, author, pageCount, bindingType,
        ...
        if (event.type == sf::Event::MouseButtonPressed) {
          if (addLibraryButton.isClicked(window)) {
            ...
            library.addItem(new Magazine(title, author, pageCount, bindingType,
                                         publicationDate, issueNumber));
        ...
        if (event.type == sf::Event::MouseButtonPressed) {
          if (addLibraryButton.isClicked(window)) {
            ...
            library.addItem(new Ebook(title, author, fileSize, format, genre,
                                      publicationDate));
      ...
      } else if (inDisplayItemMenu) {
      // Handle the display items logic
      if (event.type == sf::Event::MouseButtonPressed) {
        if (backDisplayItemsButton.isClicked(window)) {
          std::cout << "Back button clicked!" << std::endl;  // Debug output
          inDisplayItemMenu = false;  // Exit display items menu
        }
      }
    }
      ...
      if (selectedItem == 0) {
        ...
        inDisplayItemMenu = true;  // Switch to add item menu
      }
  ...
  if (inAddMemberMenu) {
    ...
    std::vector<Item *> items =
        library.getItems();  // Assuming you have a method to retrieve items

    // Display items on the screen (you might want to implement a better way to
    // show them)
    int yOffset = 50;  // Initial vertical offset for displaying items
    for (Item *item : items) {
      // Create a string representation of each item
      std::string itemInfo =
          item->getTitle() + " by " +
          item->getAuthor();  // Adjust based on your Item class
                              // Assuming each item has a method to get its info
      sf::Text itemText(itemInfo, font, 24);
      itemText.setPosition(100, yOffset);
      window.draw(itemText);
      yOffset += 30;  // Increase offset for next item
    }

    backDisplayItemsButton.draw(window);  // Draw the back button
  } else {
    // Draw main menu
    menu.draw();
  }
