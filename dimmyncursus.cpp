...
int main() {
  ...
  while (true) {
    ...
    while (true) {
        ...
        Item** items = library->getItemList();  // Assuming getItemList() returns Item**
        int numItems = library->getItemSize();  // Get the number of items

        mvprintw(maxY / 2 + 6, (maxX - 26) / 2, "List of Items:\n");

        if (numItems == 0) {
            mvprintw(maxY / 2 - 3, (maxX - 30) / 2, "No items found in the library.");
        } else {
        ...
        if (itemChoice == 0) {
            ...
            clear();
            Item* selectedItem = items[itemChoice - 1];  // Get the selected item
            
            // Display detailed information based on the item type
            mvprintw(maxY / 2 - 5, (maxX - 40) / 2, "Item Details:\n");
            mvprintw(maxY / 2 - 4, (maxX - 50) / 2, "Title: %s", selectedItem->getTitle().c_str());
            mvprintw(maxY / 2 - 3, (maxX - 50) / 2, "Author: %s", selectedItem->getAuthor().c_str());
            ...
            if (dynamic_cast<Book*>(selectedItem)) {
                ...
                mvprintw(maxY / 2 - 2, (maxX - 50) / 2, "Page Count: %d", book->getPageCount());
                mvprintw(maxY / 2 - 1, (maxX - 50) / 2, "Binding Type: %s", book->getBindingType().c_str());
                mvprintw(maxY / 2, (maxX - 50) / 2, "Genre: %s", book->getGenre().c_str());
                mvprintw(maxY / 2 + 1, (maxX - 50) / 2, "Publication Date: %s", book->getPublicationDate().c_str());
            } else if (dynamic_cast<Magazine*>(selectedItem)) {
                Magazine* magazine = dynamic_cast<Magazine*>(selectedItem);
                mvprintw(maxY / 2 - 2, (maxX - 50) / 2, "Issue Number: %d", magazine->getIssueNumber());
                mvprintw(maxY / 2 - 1, (maxX - 50) / 2, "Page Count: %d", magazine->getPageCount());
                mvprintw(maxY / 2, (maxX - 50) / 2, "Binding Type: %s", magazine->getBindingType().c_str());
                mvprintw(maxY / 2 + 1, (maxX - 50) / 2, "Publication Date: %s", magazine->getPublicationDate().c_str());
            } else if (dynamic_cast<Ebook*>(selectedItem)) {
                Ebook* ebook = dynamic_cast<Ebook*>(selectedItem);
                mvprintw(maxY / 2 - 2, (maxX - 50) / 2, "File Size: %d MB", ebook->getfileSize());
                mvprintw(maxY / 2 - 1, (maxX - 50) / 2, "Format: %s", ebook->getFormat().c_str());
                mvprintw(maxY / 2, (maxX - 50) / 2, "Genre: %s", ebook->getGenre().c_str());
                mvprintw(maxY / 2 + 1, (maxX - 50) / 2, "Publication Date: %s", ebook->getPublicationDate().c_str());
            }

            mvprintw(maxY / 2 + 2, (maxX - 30) / 2, "Press any key to go back...");
            refresh();
            getch();  // Wait for the user to press a key
        } else {
            // Invalid choice
            mvprintw(maxY / 2 + numItems + 3, (maxX - 30) / 2, "Invalid choice! Press any key to try again...");
            refresh();
            getch();  // Wait for the user to press a key
        }
    }
